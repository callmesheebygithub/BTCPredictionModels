...
class DatabaseManager:
    ...
    def _ensure_columns(self, cursor, required_columns=None):
        ...
        if required_columns is not None:
            ...
            if missing_definitions:
                raise ValueError(
                    "Unsupported DataFrame columns not present in expected schema: "
        ...
        if added:
            logger.info(
                "Auto-migrated missing database columns: %s",
                ", ".join(added)
            )

    def _create_table_from_schema(self, cursor, table_name):
        expected = self._expected_schema()
        columns = [f"`{name}` {definition}" for name, definition in expected.items()]
        columns += [
            "INDEX idx_date (`date`)",
    ...
    def _ensure_columns(self, cursor, required_columns=None):
        expected = self._expected_schema()
        if required_columns is not None:
            unsupported = [c for c in required_columns if c not in expected]
            if unsupported:
                raise ValueError(
                    "Unsupported DataFrame columns not present in expected schema: "
                    + ", ".join(unsupported)
        ...
        for column, definition in expected.items():
            if column not in existing:
                cursor.execute(
                    f"ALTER TABLE `{TARGET_TABLE}` ADD COLUMN `{column}` {definition}"
                )
                added.append(column)
        if added:
            logger.info("Auto-migrated missing database columns: %s", ", ".join(added))

    def create_table(self):
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            self._create_table_from_schema(cursor, TARGET_TABLE)
            self._ensure_columns(cursor)
            conn.commit()
            cursor.execute(f"SHOW COLUMNS FROM `{TARGET_TABLE}`")
            actual = {row[0] for row in cursor.fetchall()}
            missing = [c for c in self._expected_schema() if c not in actual]
            if missing:
                raise RuntimeError("Schema verification failed; missing: " + ", ".join(missing))
            logger.info(f"{TARGET_TABLE} verified with {len(actual)} columns.")
        finally:
            cursor.close()
            conn.close()

    def replace_table_safely(self, df):
        """Rebuild the indicator table using a staging table with the COMPLETE schema."""
        staging = f"{TARGET_TABLE}_staging"
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            # Validate that every DataFrame field is part of our production schema.
            self._ensure_columns(cursor, required_columns=list(df.columns))
            conn.commit()

            cursor.execute(f"DROP TABLE IF EXISTS `{staging}`")

            # CRITICAL: do NOT use CREATE TABLE ... LIKE TARGET_TABLE.
            # The production table may have an older schema. Build staging directly
            # from the canonical expected schema so rsi/ma_trend/etc. can never be missing.
            self._create_table_from_schema(cursor, staging)
            conn.commit()

            cursor.execute(f"SHOW COLUMNS FROM `{staging}`")
            staging_columns = {row[0] for row in cursor.fetchall()}
            missing = [c for c in df.columns if c not in staging_columns]
            if missing:
                raise RuntimeError("Staging schema missing DataFrame columns: " + ", ".join(missing))

            self.insert_batch(df, table_name=staging, connection=conn)

            cursor.execute(f"DROP TABLE IF EXISTS `{TARGET_TABLE}_old`")
            cursor.execute(
                f"RENAME TABLE `{TARGET_TABLE}` TO `{TARGET_TABLE}_old`, `{staging}` TO `{TARGET_TABLE}`"
            )
            cursor.execute(f"DROP TABLE IF EXISTS `{TARGET_TABLE}_old`")
            conn.commit()
